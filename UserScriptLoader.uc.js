...
USL.ScriptEntry.prototype = {
	...
	isURLMatching: function(url) {
		return this.includeRegExp.test(url) &&
...
USL.createMenuitem = function () {
	if (USL.popup.lastChild != USL.menuseparator) {
		var range = document.createRange();
		range.setStartAfter(USL.menuseparator);
		range.setEndAfter(USL.popup.lastChild);
...
USL.saveScript = function() {
	...
	var filename = (name && name[1] ? name[1].toLowerCase().replace( /\s/g, "_") : win.location.href.split("/").pop()).replace(/\.user\.js$|$/i, ".user.js");
...
USL.onPopupShowing = function(event) {
	...
	switch(popup.id) {
		case 'UserScriptLoader-popup':
			let run = win.USL_run;
			let show = win.USL_include;
			Array.slice(popup.children).forEach(function(menuitem){
				if (menuitem.classList.contains("UserScriptLoader-item")) {
					let index = show ? show.indexOf(menuitem.script) : -1;
					menuitem.style.fontWeight = run.indexOf(menuitem.script) !== -1 ? "bold" : "";
...
USL.injectScripts = function(safeWindow, rsflag) {
	...
	USL.readScripts.filter(function(script, index) {
		...
		safeWindow.USL_include.push(script);
...
USL.getContents_new = function(aURL, aCallback){
	try {
		urlSecurityCheck(aURL, gBrowser.contentPrincipal,Ci.nsIScriptSecurityManager.DISALLOW_INHERIT_PRINCIPAL);
	} catch(ex) {
		return;
